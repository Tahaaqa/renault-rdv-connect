...
function Reclamations() {
  ...
  const mine = reclamations.filter((r) => r.clientId === currentClientId);
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">{mine.length} réclamation(s) au total</p>
        <button
      ...
      {open && (
        <div className="rounded-xl border border-yellow/40 bg-card p-5 animate-fade-up">
          <h3 className="font-display text-base font-semibold">Décrivez votre réclamation</h3>
          <textarea
            value={text}
          ...
          <div className="mt-3 flex justify-end gap-2">
            ...
            <button onClick={submit} className="rounded-md bg-yellow px-4 py-1.5 text-sm font-semibold text-renault-black">
              Envoyer
            </button>
          </div>
        </div>
      ...
      {mine.length === 0 ? (
        <EmptyState icon={MessageSquareWarning} title="Aucune réclamation" description="Vous n'avez ouvert aucune réclamation à ce jour." />
      ) : (
            ...
            <div key={r.id} className="rounded-xl border border-border bg-card p-5 hover-lift">
              ...
              {r.resolution && (
                <div className="mt-3 rounded-md border border-yellow/20 bg-yellow/5 p-3 text-sm">
                  <div className="mb-1 text-[11px] font-semibold uppercase tracking-widest text-yellow">Réponse Renault</div>
                  {r.resolution}
                </div>
              )}
            </div>

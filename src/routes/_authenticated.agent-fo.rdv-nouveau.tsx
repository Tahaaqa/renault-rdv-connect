...
function AFONouveau() {
  ...
  const { clients: localClients, vehicules: localVehicules, createRDV } = useDataStore();
  ...
  const agenceId = profile?.agenceId ?? agences[0]?.id ?? SEED_AGENCES[0].id;
  ...
  const mutation = useMutation({
    ...
    onError: (_error, input) => {
      createRDV({
        clientId: input.clientUserId ?? clientId,
        agenceId: input.agencyId,
        vehiculeId: input.vehicleId,
        date: input.startsAt,
        notes: input.notes ?? undefined,
        statut: "Confirme",
      });
      toast.success("RDV cree localement");
    },
  });
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div className="rounded-xl border border-border bg-card p-6">
        ...
        <button onClick={submit} className="press mt-6 inline-flex rounded-md bg-yellow px-5 py-2 text-sm font-semibold text-renault-black">
